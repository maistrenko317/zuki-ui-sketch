...
export class GameComponent implements OnInit, AfterViewInit {
    ...
    get gameType(): GameType {
        return this.game.productionGame ? 'prod' : (this.game.gameType === 'TESTER' ? 'test' : 'demo');
    }
    ...
    async validate(): Promise<void> {
        ...
        if (this.form.valid) {
            ...
            if (this.game.productionGame) {
                const areYouSure = this.vxDialog.open(AlertDialogComponent, {
                    title: 'Confirm - Important',
                    body: `This game is setup as a production game.  This means that real payouts will be used.  Continue?`,
                    buttons: ['Yes', 'No']
                });
                const resp = await areYouSure.onClose.toPromise();
                if (resp === 'No') {
    ...
    setGameType(type: GameType): void {
        this.game.productionGame = type === 'prod';
        this.game.gameType = type === 'test' ? 'TESTER' : 'DEFAULT';
    }
    ...
    private initializeGame(): void {
        ...
        game.engineType = 'FixedRound_MultiLife';
