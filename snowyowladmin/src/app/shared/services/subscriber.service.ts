...
export class SubscriberService {
    ...
    async loadIneligibleSubscribers(): Promise<void> {
        const url = environment.domains.zukiURL + "/subscriber/ineligible"
        const resp = await this.httpService.getZukiResponse<any>(url).toPromise();        
        let keys = Object.keys(resp);

        if(keys.length > 1) {
            keys = keys.slice(0, keys.length - 1)
            const array = keys.reduce((result: IneligibleSub[], key: string) => [...result, resp[key]], []);
            this.ineligibleSubs = array;
        } else {
            this.ineligibleSubs = [];
        }
    }

    async addIneligibleSubscriber(email: string, reason: string, linkedEmail?: string): Promise<IneligibleSubsResponse> {
        const url = environment.domains.zukiURL + "/subscriber/ineligible"
        return await this.httpService.postZukiForm<IneligibleSubsResponse>(url, {email, reason}).toPromise();
    }
