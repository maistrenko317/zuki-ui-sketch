...
export class HttpService {
    ...
    getZukiResponse<T extends ShoutResponse>(url: string, body: any = {}, ignore401 = false, deviceInfo = false): Observable<T> {
        return this.srd.pipe(filter((val): val is SRD => !!val), take(1), mergeMap((srd: SRD) => {
            return this.http.get<ZukiResponse<ShoutResponse>>(url, {headers: this.getHeaders(deviceInfo)})
            .pipe(    
                map(response => {
                    const entity = response.entity as T;
    ...
    postZukiForm<T extends ShoutResponse>(url: string, body: any = {}, ignore401 = false, deviceInfo = false): Observable<T> {
        const params = new HttpParams({fromObject: body})
        return this.srd.pipe(filter((val): val is SRD => !!val), take(1), mergeMap((srd: SRD) => {
            return this.http.post<ZukiResponse<ShoutResponse>>(url, params, {headers: this.getFormHeaders(deviceInfo)})
            .pipe(    
                map(response => {
                    const entity = response.entity as T;
                    return ({...entity, success: true});
    ...
    public getHeaders(deviceInfo: boolean): HttpHeaders {
    ...
    public getFormHeaders(deviceInfo: boolean): HttpHeaders {
        const headers = this.getHeaders(deviceInfo);
